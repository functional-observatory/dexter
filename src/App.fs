...
let withLoadPokemon pokeId state =
  state, Cmd.OfAsync.perform getPokemon pokeId id
...
let update (msg: Msg) (state: State) =
  match msg with
  | GetData pokeId ->
      { state with ResponseText = Loading }
      |> withLoadPokemon pokeId
  | DataLoaded data ->
      { state with
